...
/**
 * @author Yushan
 * @date 2021/9/8 4:12 下午
 */
@SdkExtension(bizId = ScenarioConstantApp.BIZ_TYPE_SUPERMARKET,
        useCase = ScenarioConstantApp.LOC_TYPE_B2C,
        scenario = ScenarioConstantApp.B2C_CNXH_CONTENT_FEEDS)
@Service
public class GuessWhatYouLikeMenuContentInfoQueryExtPt extends Register implements ContentInfoQuerySdkExtPt {
    ...
    public Flowable<Response<Map<Long, ContentInfoDTO>>> process(SgFrameworkContextContent sgFrameworkContextContent) {
        ...
        try {
            ...
            HadesLogUtil.stream(ScenarioConstantApp.B2C_CNXH_CONTENT_FEEDS)
                    .kv("userId",Optional.of(sgFrameworkContextContent)
                            .map(SgFrameworkContextContent::getCommonUserParams)
                            .map(CommonUserParams::getUserDO)
                            .map(UserDO::getUserId)
                            .map(Objects::toString)
                            .orElse("0"))
                    .kv("B2CCNXHContentInfoQueryExtPt","process")
