...
public class AliPaySuccessGuessYouLikeItemProcessBeforeReturnSdkExtPt extends DefaultItemProcessBeforeReturnSdkExtPt implements ItemProcessBeforeReturnSdkExtPt {
    ...
    public SgFrameworkContextItem process(SgFrameworkContextItem sgFrameworkContextItem) {
        ...
        tacLogger.info("返回前处理内容前信息：" + JSON.toJSONString(sgFrameworkContextItem));

        SgFrameworkContextItem process = super.process(sgFrameworkContextItem);

        tacLogger.info("返回前处理内容后信息：" + JSON.toJSONString(sgFrameworkContextItem));
        return process;
    }
}
